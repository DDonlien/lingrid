...
- 每条日志还应包含：
  ...
  - 验证方式
  - 备注
- 版本号维护规范：每次执行完任务，必须判断是否需要更新版本号（采用 3 位 `x.x.x` 格式）。如果是，则按以下原则更新并同步在执行日志中明确记录：
  - 原则上每个需求大 Section（如 Epic）的实现或录入对应 `minor` 版本升级。
  - 普通变更、Bug 修复、小型调整对应 `patch` 版本升级。
  - `major` 版本由用户手动定义。
  - 更新版本号时，必须同步修改 `package.json` 的 `version` 属性与前端 UI 界面展示的版本号。
