...
- [ ] [AI-A-090] AI 设置支持多模型并发与失败转发 #feature #P1.5
  - [ ] [AI-A-091] AI 设置 TAB 中的模型/接口支持多选
  ...
  - [ ] [AI-A-092] 多选模型后的并发执行与 source 分配策略
...
- [ ] [FILE-B-000] 隐藏式项目管理与合并授权自动加载 #feature #P1.5
  - [ ] [FILE-B-001] 项目配置文件改名/重构为隐藏文件样式（如 `.lingrid.json`，存储于目标翻译文件夹中）
  - [ ] [FILE-B-002] 合并文件夹选择与目录授权：加载项目时直接要求选择并一次性以 `readwrite` 权限授权目标项目文件夹，应用自动在根目录下搜寻并读取 `.lingrid.json` 配置文件，匹配成功即直接载入项目
  - [ ] [FILE-B-003] 新建项目或保存项目时，在目标已授权目录根节点下自动静默写入/更新 `.lingrid.json` 配置文件
  - [ ] [FILE-B-004] 保留完整的目录授权状态与句柄引用，以便在快速打开历史（已有项目）时能够自动重用句柄或引导顺畅授权

### FILE-C：最近项目历史记录列表 #epic #P1.5

- [ ] [FILE-C-000] 基于 localStorage 的已知项目快速访问 #feature #P1.5
  - [ ] [FILE-C-001] 在 `localStorage` 中记录已打开项目的历史工程列表
    - [ ] 记录项目路径（或文件夹名称）、上次访问时间
    - [ ] 每次成功打开项目时更新该记录，将最近访问的项目置顶
    - [ ] 历史列表上限待定（建议 10–20 条），超出时淘汰最旧的记录
  - [ ] [FILE-C-002] "打开项目"按钮改为 split button（主按钮 + 下拉箭头）
    - [ ] 点击主按钮（左侧"打开项目"文字区域）→ 保持现有行为，弹出文件夹/项目选择器
    - [ ] 点击右侧下拉箭头 → 展开最近项目下拉菜单
    - [ ] 下拉菜单中列出最近打开过的项目记录，每条显示文件夹名称、路径摘要、上次访问时间
    - [ ] 点击下拉菜单中的某条记录 → 直接打开对应项目，不再弹出选择器
    - [ ] 下拉菜单支持点击外部区域自动收起
  - [ ] [FILE-C-003] 最近项目列表的交互与边界处理
    - [ ] 支持单条移除：hover 某条记录时显示移除按钮，点击后从 localStorage 删除该条历史
    - [ ] 支持清空全部历史：列表底部提供"清空历史"入口，点击后确认再清除
    - [ ] 空状态：当没有历史记录时，下拉菜单显示"暂无最近项目"提示
    - [ ] 失效项目处理：若记录中的项目路径已不存在或不可访问，打开时提示用户并询问是否从列表中移除该记录
    - [ ] 浏览器版与桌面版共用同一套 localStorage 历史记录格式，但路径语义各自兼容（浏览器存目录句柄标识或路径，桌面版存绝对路径）
